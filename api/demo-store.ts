...
function requireToken(res: VercelResponse): string | null {
  const token = getToken();
  if (!token) {
    res.status(500).json({
      error: 'Vercel Blob is not configured. Add BLOB_READ_WRITE_TOKEN to the production environment and redeploy.',
    });
    return null;
  }
  return token;
...
async function blobList(token: string, prefix: string) {
  ...
  try { payload = JSON.parse(text); } catch { /* preserve raw response */ }
  if (!response.ok) {
    throw new Error(payload?.error?.message || text || `Blob list failed (${response.status}).`);
  }
...
async function blobPut(token: string, pathname: string, body: string) {
  ...
  try { payload = JSON.parse(text); } catch { /* preserve raw response */ }
  if (!response.ok) {
    throw new Error(payload?.error?.message || text || `Blob write failed (${response.status}).`);
  }
...
async function blobDelete(token: string, urls: string[]) {
  ...
  try { payload = JSON.parse(text); } catch { /* preserve raw response */ }
  if (!response.ok) {
    throw new Error(payload?.error?.message || text || `Blob delete failed (${response.status}).`);
  }
}
...
function validateClient(value: unknown): value is Record<string, any> {
...
function applyNoStore(res: VercelResponse) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (action === 'list') {
      ...
      const jsonBlobs = (listed.blobs || []).filter((item) => item.pathname.endsWith('.json'));
      const clients = await Promise.all(
        jsonBlobs.map(async (blob) => {
          try {
            const value = await fetchJson(blob.url);
            return validateClient(value) ? value : null;
          } catch {
            return null;
          }
        }),
      );
      return res.status(200).json({ data: clients.filter(Boolean) });
    }
    ...
    if (action === 'save') {
      ...
      if (existing && mode === 'create' && existing.client.id !== client.id) {
        return res.status(409).json({ error: 'That demo slug already exists. Choose a different business name/slug.' });
      }
