...
export default async function handler(req: any, res: any) {
  ...
  try {
    const response = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent', {
      ...
      body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }], generationConfig: { responseMimeType: 'application/json', responseSchema, temperature: 0.7 } }),
    });
    if (!response.ok) { console.error('Gemini API error:', await response.text()); return res.status(502).json({ error: 'Gemini could not generate the demo content. Please try again.' }); }
