  ...
  return (
    <div className="min-h-[100svh] bg-[#0a0a0f] text-white flex items-center justify-center px-5">
      <div className="w-full max-w-md text-center rounded-3xl border border-white/10 bg-white/[0.03] p-8 sm:p-10 shadow-2xl">
        <div className="mx-auto mb-5 flex h-12 w-12 items-center justify-center rounded-full bg-amber-400/10 text-amber-300 text-xl">{expired ? '!' : '×'}</div>
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight mb-3">{expired ? 'Demo expired' : 'Demo unavailable'}</h1>
        <p className="text-sm leading-6 text-white/55">{error || 'This demo link is invalid or could not be loaded.'}</p>
      </div>
    </div>
  );
}
