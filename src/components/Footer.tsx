    ...
    >
      <Container>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              {client.location?.address && (
                <span className="flex items-center gap-2 text-sm" style={{ color: 'var(--brand-muted)' }}>
                  <MapPin size={16} style={{ color: 'var(--brand-accent)' }} />
                  {client.location.address}
                </span>
              )}
              {c?.phone && (
                <a href={`tel:${c.phone.replace(/\s/g, '')}`} className="flex items-center gap-2 text-sm" style={{ color: 'var(--brand-muted)' }}>
                  <Phone size={16} style={{ color: 'var(--brand-accent)' }} />
                  {c.phone}
                </a>
              )}
              {c?.email && (
                <a href={`mailto:${c.email}`} className="flex items-center gap-2 text-sm" style={{ color: 'var(--brand-muted)' }}>
                  <Mail size={16} style={{ color: 'var(--brand-accent)' }} />
                  {c.email}
                </a>
              )}
              {c?.instagram && (
                <a
                  ...
                  style={{ color: 'var(--brand-muted)' }}
                >
                  <Instagram size={16} style={{ color: 'var(--brand-accent)' }} />
                  {c.instagram}
                </a>
              )}
              {c?.website && (
                <a href={c.website.startsWith('http') ? c.website : `https://${c.website}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm" style={{ color: 'var(--brand-muted)' }}>
                  <Globe size={16} style={{ color: 'var(--brand-accent)' }} />
                  {c.website}
                </a>
              )}
            </div>
        ...
        <div
          ...
          style={{ borderColor: 'rgba(var(--brand-muted-rgb), 0.08)' }}
        >
          <p className="text-xs" style={{ color: 'var(--brand-muted)' }}>
            &copy; {year} {client.businessName}. All rights reserved.
          </p>
          <p className="text-xs" style={{ color: 'var(--brand-muted)' }}>
            Powered by Horizon Works Demo Engine
          </p>
        </div>
      </Container>
    </footer>
