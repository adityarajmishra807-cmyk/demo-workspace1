      ...
      <Container>
        <div className="relative overflow-hidden rounded-[2rem] border px-7 py-16 md:px-16 md:py-24" style={{ borderColor: 'var(--brand-accent-border)', background: 'var(--brand-surface)' }}>
          ...
          <div className="relative z-10 max-w-4xl">
            ...
            <h2 className="text-4xl md:text-6xl lg:text-7xl font-medium leading-[1.02] mb-7" style={{ color: 'var(--brand-surface-text)' }}>{client.headline || `Ready to work with ${client.businessName}?`}</h2>
            <p className="text-lg md:text-xl leading-8 max-w-2xl mb-10" style={{ color: 'var(--brand-surface-muted)' }}>{client.tagline || 'Get in touch today and let\'s discuss how we can help.'}</p>
          ...
          <div className="absolute right-10 bottom-8 hidden md:block text-[9rem] leading-none font-medium opacity-[0.035] select-none" style={{ color: 'var(--brand-surface-text)' }}>→</div>
        </div>
      </Container>
    </Section>
  );
