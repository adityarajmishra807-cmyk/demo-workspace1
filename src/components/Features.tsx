      ...
      <Container>
        <div className="grid lg:grid-cols-[0.8fr_1.2fr] gap-14 lg:gap-24 items-start">
          <div className="lg:sticky lg:top-32">
            ...
            <h2 className="text-5xl md:text-6xl font-medium leading-[1.05] mb-6" style={{ color: 'var(--brand-text)' }}>Why it feels different.</h2>
            <p className="text-base md:text-lg leading-8 max-w-md" style={{ color: 'var(--brand-muted)' }}>The details behind a considered experience matter just as much as the destination.</p>
          </div>
          <div className="divide-y border-t" style={{ borderColor: 'rgba(var(--brand-muted-rgb), 0.16)' }}>
            {client.features.map((feature, i) => {
              ...
              return (
                <div key={i} className="group grid grid-cols-[56px_1fr] gap-6 py-8 md:py-10 transition-transform duration-300 hover:translate-x-2">
                  ...
                  <div>
                    <div className="flex items-baseline gap-4 mb-2"><span className="text-xs tracking-widest" style={{ color: 'var(--brand-accent)' }}>{String(i + 1).padStart(2, '0')}</span><h3 className="text-xl md:text-2xl font-medium" style={{ color: 'var(--brand-text)' }}>{feature.title}</h3></div>
                    <p className="text-base leading-7 max-w-xl" style={{ color: 'var(--brand-muted)' }}>{feature.description}</p>
                  </div>
                </div>
              );
            })}
