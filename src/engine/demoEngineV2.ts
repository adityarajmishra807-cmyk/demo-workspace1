...
import { buildContentPlan, contentDepthFor, type ContentPlan } from './contentIntelligence';
...
import { buildDesignTokens, type DesignTokens } from './designSystem';
import { buildLongFormPlan, type LongFormPlan } from './longForm';
import { runQualityGate, type QualityReport } from './qualityGate';
import { resolveTemplateV2, type TemplateV2Blueprint } from '@/templates/templateV2';
...
export function buildDemoEngineV2(client: ClientConfig): DemoEngineV2Result {
  ...
  const design = buildDesignTokens(profile, archetype);
  const longForm = buildLongFormPlan(client, profile, archetype);
  ...
  const mobile = buildMobileStrategy(contentDepthFor(content, archetype) === 'rich' ? 'immersive' : 'balanced');
  const conversion = buildConversionPlan(profile, archetype);
  const template = resolveTemplateV2(client);
...
export function canRenderDemoV2(result: DemoEngineV2Result): boolean {
