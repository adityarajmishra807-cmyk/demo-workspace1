...
export default function Dashboard({ navigate }: { navigate: (to: string) => void }) {
  ...
  const handleDelete = () => {
    ...
    try { deleteClient(deleteTarget.id); setDeleteTarget(null); refresh(); } catch { setDeleteTarget(null); }
  };

  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      <header className="border-b border-white/10"><div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-gradient-to-br from-sky-400 to-sky-600 flex items-center justify-center"><LayoutDashboard size={22} className="text-white" /></div><div><h1 className="text-lg font-bold tracking-tight">Horizon Works</h1><p className="text-xs text-white/50">Demo Dashboard</p></div></div><a href="#/" className="flex items-center gap-2 text-sm text-white/60 hover:text-white transition-colors"><ArrowLeft size={16} /> Home</a></div></header>
      <main className="max-w-6xl mx-auto px-6 py-10">
        <div className="mb-10"><h2 className="text-2xl md:text-3xl font-bold mb-2">Client Demos</h2><p className="text-white/50 text-sm max-w-2xl">This is the internal dashboard for managing client website demos. Each demo is generated from a configuration file, not a separate codebase.</p></div>
