...
import { LayoutDashboard, Plus, ExternalLink, ArrowLeft, Pencil, Trash2, X, Copy, Check, RefreshCw, Eye } from 'lucide-react';
...
export default function Dashboard({ navigate }: { navigate: (to: string) => void }) {
  ...
  const handleOpenPublic = (id: string) => {
    const client = getClientById(id); if (!client) return;
    window.open(`https://${client.slug}.demo.horizonworks.co.in/`, '_blank', 'noopener,noreferrer');
  };
